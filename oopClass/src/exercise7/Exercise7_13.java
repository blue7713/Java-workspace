...
class Machine{
	...
	public void execute() {
		...
		while(true) {
			...
			// printVariables(intruction); 
			// 이 메소드를 실행하면 실행 중에 변하는 변수 모두 볼 수 있음
		}
	}
	...
	private void prt(Instruction instruction) { // 첫번째 피연산자 값을 출력하고 종료. 두번째 피연산자는 의미 없음
	...
	private int jn0(Instruction instruction) { // 첫번재 피연산자가 0이 아니면 두번째 피연산자의 주소로 점프
		String first = instruction.getOperand(0);
	...
	private int getValue(String opr) { // opr이 정수아면 수로 리턴. 변수명이면 변수명의 값 리턴. 없는 변수이면 새로만들고 0리턴
		int n;
		try {
