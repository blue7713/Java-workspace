...
public class SliderChangeEx extends JFrame{
	...
	public SliderChangeEx() {
		...
		colorLabel = new JLabel("		SLIDER EXAMPLE		"); // 3개의 슬라이더에서 선택한 색을 조합하여 이 컴포넌트의 배경으로 설정함
		
		// 슬라이더 3개 생성하고 모양을 제어한다.
		...
		sl[2].setForeground(Color.BLUE); // s[2] 슬라이더는 BLUE 값 선택
		
		// 현재 3개의 슬라이더로부터 값을 얻어 초기 colorLabel의 배경색 설정
		int r = sl[0].getValue(); // 첫 번째 슬라이더로부터 R 성분 값 얻기
		int g = sl[1].getValue(); // 두 번쨰 슬라이더로부터 G 성분 값 얻기
		int b = sl[2].getValue(); // 세 번쨰 슬라이더로부터 B 성분 값 얻기
		colorLabel.setOpaque(true); // 배경색이 출력되도록 불투명성 결정
	...
	class MyChangeListener implements ChangeListener{
		public void stateChanged(ChangeEvent e) { // 슬라이더의 값이 변경될 때 호출
